...
const CandidateInvite: React.FC = () => {
  ...
  const fetchOrganizations = async () => {
    try {
      ...
      if (Array.isArray(organizationsData)) {
        ...
        console.log("✅ Set organizations as array:", organizationsData);
      } else if (organizationsData && typeof organizationsData === 'object' && 'results' in organizationsData && Array.isArray((organizationsData as any).results)) {
        // Handle paginated response structure
        const results = (organizationsData as any).results;
        setOrganizations(results);
        console.log("✅ Set organizations from results:", results);
      } else {
        console.warn("⚠️ Organizations response is not an array:", organizationsData);
        setOrganizations([]);
      }
    } catch (err) {
  ...
  const handleBulkUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${getApiUrl()}/profiles/api/candidate-onboarding/bulk-invite/`, {
        method: 'POST',
        headers: {
