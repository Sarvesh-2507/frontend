...
import { User, Calendar, FileText, DollarSign, Users, BookOpen, UserCircle } from 'lucide-react';
    ...
    <aside className="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 min-h-screen p-4 flex flex-col items-center relative">
      ...
      >
        <div className="w-14 h-14 rounded-full bg-blue-600 flex items-center justify-center shadow-lg border-4 border-blue-200 dark:border-blue-800 mb-1">
          <span className="text-white text-xl font-bold">{profile?.first_name ? (profile.first_name[0] + (profile.last_name ? profile.last_name[0] : '')) : 'EMP'}</span>
        </div>
        <span className="text-xs text-blue-700 dark:text-blue-200 font-semibold group-hover:underline">Profile</span>
      </button>
      {/* Sidebar Navigation */}
      <nav className="space-y-2 w-full">
        {sidebarItems.map(item => (
          <Link
            key={item.label}
            to={item.path}
            className={`flex items-center gap-3 px-4 py-2 rounded-lg transition-colors font-medium text-gray-700 dark:text-gray-200 hover:bg-blue-50 dark:hover:bg-blue-900 ${location.pathname === item.path ? 'bg-blue-100 dark:bg-blue-700 text-blue-700 dark:text-white' : ''}`}
          >
