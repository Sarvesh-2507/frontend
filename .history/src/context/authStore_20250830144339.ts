...
const isTokenValid = (token: string): boolean => {
  ...
  try {
    ...
    if (token.includes("_refresh")) {
      return true; // Mock refresh tokens are always valid
    }
    ...
    // For mock tokens, always consider them valid if they have the right structure
    if (payload.exp && payload.iat) {
      return payload.exp > currentTime;
    }
    
    // Fallback for any token format
    return true;
  } catch (error) {
    console.error('Token validation error:', error);
    // For development, be permissive with token validation
      ...
      checkSession: async () => {
        try {
            ...
            if (refreshToken && isTokenValid(refreshToken)) {
              try {
                ...
                if (refreshResponse.success && refreshResponse.data) {
                  const newTokens = {
                    access: refreshResponse.data.access || accessToken,
                  ...
                  setStoredTokens(newTokens.access, newTokens.refresh);
                  return true;
                }
              } catch (refreshError) {
          ...
          try {
            ...
            // Don't logout on user fetch failure, token might still be valid
          }
