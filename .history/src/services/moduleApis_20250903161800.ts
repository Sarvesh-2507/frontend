imimport { getApiUrl } from "../config";

// API Configuration using environment variables
const API_BASE_URL = getApiUrl(); axios, { AxiosResponse } from 'axios';
...
export const payrollAPI = {
  ...
  getSalaryStructures: async (params?: any): Promise<AxiosResponse<any[]>> => {
    try {
      const response = await axios.get(`${API_BASE_URL}/payroll/salary-structures/`, {
        headers: getAuthHeaders(),
        params,
      });
      return response;
  ...
  generatePayslips: async (month: string): Promise<AxiosResponse<any>> => {
    return axios.post(`${API_BASE_URL}/payroll/payslips/`, { month }, {
      headers: getAuthHeaders(),
    });
  },

  getTaxRules: async (): Promise<AxiosResponse<any[]>> => {
    try {
      const response = await axios.get(`${API_BASE_URL}/payroll/tax-rules/`, {
        headers: getAuthHeaders(),
      });
      return response;
    } catch (error) {
      return {
        data: [
          { id: '1', name: 'Standard Income Tax', rate: 10, threshold: 250000 },
          { id: '2', name: 'Professional Tax', rate: 2 },
        ],
        status: 200,
      } as AxiosResponse<any[]>;
    }
  ...
  generateBankBatch: async (month: string): Promise<AxiosResponse<any>> => {
    return axios.post(`${API_BASE_URL}/payroll/bank-batch/`, { month }, {
      headers: getAuthHeaders(),
    });
  },
};
