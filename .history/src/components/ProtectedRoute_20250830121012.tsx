...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({
  ...
  if (!isAuthenticated && !isLoading) {
    // Check localStorage/sessionStorage for token and user as a fallback
    const accessToken = localStorage.getItem("accessToken") || sessionStorage.getItem("accessToken");
    const userData = localStorage.getItem("user") || sessionStorage.getItem("user");
    if (accessToken) {
      // Optionally, restore user state if missing
      if (!user && userData) {
        try {
          // If user is not in state but present in storage, restore it
          // This assumes user is stored as JSON string
          const parsedUser = JSON.parse(userData);
          // You may want to update your store here if needed
        } catch (e) {
          // Ignore parse errors
        }
      }
      // Allow rendering children if token exists
      return (
  ...
  // If a child component (like Inbox/Announcements) gets a 401 from an API, it should show its own fallback UI, not trigger a redirect here.
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
