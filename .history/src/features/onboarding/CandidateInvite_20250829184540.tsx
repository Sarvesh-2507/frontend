...
const CandidateInvite: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'organization') {
      console.debug(`Setting organization to: ${value} (${parseInt(value) || 0})`);
      setFormData({
        ...formData,
  ...
  const handleSendInvite = async (candidateId: string) => {
    ...
    try {
      ...
      showSuccess(`Invite sent to ${candidate.first_name} ${candidate.last_name}`);
    } catch (err) {
  ...
  const handleResendInvite = async (candidateId: string) => {
    ...
    try {
      ...
      showSuccess(`Invite resent to ${candidate.first_name} ${candidate.last_name}`);
    } catch (err) {
